...
pub struct FocusSession {
 pub id:String, pub task_id:Option<String>, pub task_title:String, pub duration_ms:u64,
 pub remaining_ms:u64, pub end_at:Option<u64>, pub status:String, pub created_at:String,
}
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct FocusNote { pub id:String, pub session_id:String, pub task_id:Option<String>, pub task_title:String, pub text:String, pub created_at:String }
...
pub struct Workspace {
    #[serde(default)] pub sidebar_collapsed:bool,
    #[serde(default)] pub focus:Option<FocusSession>,
    #[serde(default)] pub focus_notes:Vec<FocusNote>,
    pub revision: u64,
...
impl Workspace {
    pub fn validate(&self) -> Result<()> {
        let exists = |id:&Option<String>|id.as_ref().is_none_or(|id|self.tasks.iter().any(|t|&t.id==id));
        if self.tasks.iter().any(|t|!["none","low","medium","high","critical"].contains(&t.priority.as_str())) {return Err("Ungültige Priorität.".into());}
        ids(self.focus_notes.iter().map(|n|n.id.as_str()))?;
        if self.focus_notes.iter().any(|n|!exists(&n.task_id)||!valid_text(&n.session_id,128)||!valid_text(&n.text,10000)||n.task_title.chars().count()>300||!valid_text(&n.created_at,64)) {return Err("Ungültige Focus-Notiz.".into());}
        if let Some(f)=&self.focus {
            if !valid_text(&f.id,128) || !exists(&f.task_id) || f.task_title.chars().count()>300 || !(60000..=14400000).contains(&f.duration_ms) || f.remaining_ms>f.duration_ms || !["running","paused","completed","stopped"].contains(&f.status.as_str()) || (if f.status=="running" {f.end_at.is_none_or(|end|end==0||end>9007199254740991)}else{f.end_at.is_some()}) || (f.status=="completed"&&f.remaining_ms!=0) {return Err("Ungültige Focus-Zeit.".into());}
        }
...
impl Database {
    pub fn open(path: &Path) -> Result<Self> {
        ...
        if version>0 && version<3 && path!=Path::new(":memory:") {
            let stamp=std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map_err(err)?.as_nanos();
            let backup=path.with_file_name(format!("{}.before-v3-{stamp}.bak",path.file_name().unwrap_or_default().to_string_lossy()));
            conn.execute("VACUUM INTO ?1",[backup.to_string_lossy().as_ref()]).map_err(err)?;
        }
        if version == 0 {
        ...
        if version<3 {
            conn.execute_batch(&format!("BEGIN IMMEDIATE; {} COMMIT;",include_str!("migration_3.sql"))).map_err(err)?;
        }
        Ok(Self { conn })
    }
    pub fn load(&mut self) -> Result<Workspace> {
        let tx = self.conn.transaction().map_err(err)?;
        let (revision, active_project_id,sidebar_collapsed,focus_json): (u64,Option<String>,bool,Option<String>) = tx
            .query_row(
                "SELECT revision,active_project_id,sidebar_collapsed,focus_state FROM metadata WHERE id=1",
                [],
        ...
        let focus=focus_json.map(|json|serde_json::from_str(&json).map_err(err)).transpose()?;
        ...
        Ok(Workspace {
            sidebar_collapsed,focus,focus_notes,
            revision,
            active_project_id,
            projects,
            boards,
    ...
    pub fn save(&mut self, state: &Workspace) -> Result<u64> {
        ...
        for n in &state.focus_notes { tx.execute("INSERT INTO focus_notes VALUES(?1,?2,?3,?4,?5,?6) ON CONFLICT(id) DO UPDATE SET task_id=excluded.task_id,task_title=excluded.task_title,text=excluded.text",params![n.id,n.session_id,n.task_id,n.task_title,n.text,n.created_at]).map_err(err)?; }
        for (table, keep) in [
            ("focus_notes",state.focus_notes.iter().map(|n|n.id.as_str()).collect::<HashSet<_>>()),
            (
                "tasks",
                state
                    .tasks
        ...
        let next = revision + 1;
        tx.execute(
            "UPDATE metadata SET revision=?1,active_project_id=?2,sidebar_collapsed=?3,focus_state=?4 WHERE id=1",
            params![next, state.active_project_id,state.sidebar_collapsed,state.focus.as_ref().map(serde_json::to_string).transpose().map_err(err)?],
        )
        .map_err(err)?;
        tx.commit().map_err(err)?;
        Ok(next)
...
mod tests {
    ...
    fn sample() -> Workspace {
        Workspace {
            sidebar_collapsed:false,focus:None,focus_notes:vec![],
            revision: 0,
    ...
    fn persists_focus_priority_notes_and_sidebar() {
        ...
        s.focus = Some(FocusSession { id: "f".into(), task_id: Some("t".into()), task_title: "Küche".into(), duration_ms: 60000, remaining_ms: 60000, end_at: Some(120000), status: "running".into(), created_at: "now".into() });
        s.focus_notes.push(FocusNote { id: "n".into(), session_id: "f".into(), task_id: Some("t".into()), task_title: "Küche".into(), text: "Weiter planen".into(), created_at: "now".into() });
        db.save(&s).unwrap();
    ...
    fn backs_up_v2_before_migration() {
        let dir = std::env::temp_dir().join(format!("taskhub-backup-{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
        std::fs::create_dir(&dir).unwrap();
        let path = dir.join("taskhub.db");
        {
            let conn = Connection::open(&path).unwrap();
            conn.execute_batch(include_str!("schema.sql")).unwrap();
            conn.execute_batch("ALTER TABLE tasks ADD COLUMN checklist TEXT; PRAGMA user_version=2;").unwrap();
        }
        drop(Database::open(&path).unwrap());
        let backup = std::fs::read_dir(&dir).unwrap().map(|e|e.unwrap().path()).find(|p|p.extension().is_some_and(|e|e=="bak")).unwrap();
