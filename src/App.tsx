    ...
    >
      ...
      {modal?.kind === "settings" && (
        <Dialog title="Einstellungen" close={() => setModal(null)}>
          ...
          {settingsTab === "timer" && <div className="settings-section"><h3>Focus-Timer</h3><p className="muted">Wähle einen lokalen Cozy-Chime für das Ende deiner Focus-Zeit.</p><div className="sound-grid">{focusSounds.map(sound => { const selected=s.focusSound === sound.id || (!s.focusSound && sound.id === "amber"); return <button key={sound.id} className={selected ? "sound-choice selected" : "sound-choice"} onClick={() => void change(workspace => { workspace.focusSound = sound.id; })}><span><strong>{sound.name}</strong>{selected && <Check size={16} aria-label="Ausgewählt" />}</span><small>{sound.description}</small></button>})}</div><label className="timer-volume">Lautstärke <strong>{timerVolume ?? s.focusVolume ?? 55}%</strong><input type="range" min="0" max="100" step="1" value={timerVolume ?? s.focusVolume ?? 55} onChange={event => setTimerVolume(Number(event.currentTarget.value))} onPointerUp={event => { const focusVolume=Number((event.currentTarget as HTMLInputElement).value); setTimerVolume(null); void change(workspace => { workspace.focusVolume = focusVolume; }); }} onBlur={event => { if(timerVolume === null)return; const focusVolume=Number(event.currentTarget.value); setTimerVolume(null); void change(workspace => { workspace.focusVolume = focusVolume; }); }} /></label><p className="muted">Die Einstellung gilt auch für den Testton.</p><button className="primary" onClick={focus.testSound}>Ausgewählten Klang testen</button></div>}
